  ...
  <aside class="fixed top-0 left-0 w-64 h-screen bg-gray-800 text-white flex flex-col z-30 shadow-lg">
    ...
    <nav class="flex-grow p-4 space-y-2">
      ...
      <template v-if="isEstudiante">
        <h2 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-4 mb-2">Mi Juego (Nivel: {{ userLevel || 'N/A' }})</h2>

        <RouterLink :to="{ name: 'dashboard' }" class="nav-link"><span>Dashboard</span></RouterLink>

        <RouterLink v-if="isSemilla" :to="{ name: 'lecciones-semilla' }" class="nav-link">
            <span>Lecciones (Semilla)</span>
        </RouterLink>

        <RouterLink v-if="isBrote" :to="{ name: 'presupuesto-brote' }" class="nav-link">
            <span>Módulo Presupuesto (Brote)</span>
        </RouterLink>
        <RouterLink v-if="isBrote" :to="{ name: 'kiosco' }" class="nav-link">
            <span>Tienda Virtual (Brote)</span>
        </RouterLink>
      </template>
